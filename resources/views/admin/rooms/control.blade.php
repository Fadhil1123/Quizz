    ...
    <div class="max-w-7xl mx-auto space-y-6">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                <!-- Card Soal Aktif -->
                <div class="bg-slate-900 border border-pink-500/30 p-6 rounded-2xl space-y-4 shadow-xl shadow-pink-500/5">
                    ...
                    @if($activeQuestion)
                        ...
                        <div class="pt-4 border-t border-slate-800 space-y-3">
                            <h3 class="text-xs font-bold text-slate-400 uppercase">Aksi Transaksi Tim</h3>
                                    ...
                                    <div class="bg-slate-950 p-3 rounded-xl border border-slate-800 flex justify-between items-center">
                                        ...
                                        <div class="flex gap-1">
                                            ...
                                            <form action="/admin/rooms/{{ $room->id }}/action" method="POST">
                                                ...
                                                <input type="hidden" name="question_id" value="{{ $activeQuestion->masterQuestion->id }}">
                                                <input type="hidden" name="room_question_id" value="{{ $activeQuestion->id }}">
                                                <button class="px-2.5 py-1 bg-blue-500/20 text-blue-300 border border-blue-500/30 rounded-lg text-xs font-bold hover:bg-blue-500/30">Operan (+1x)</button>
                                            </form>
                                        </div>
                                    </div>
                                @endforeach
                            ...
                            <form action="/admin/rooms/{{ $room->id }}/action" method="POST" class="pt-2">
                                ...
                                <input type="hidden" name="question_id" value="{{ $activeQuestion->masterQuestion->id }}">
                                <input type="hidden" name="room_question_id" value="{{ $activeQuestion->id }}">
                                <button class="w-full py-2 bg-slate-800 text-slate-400 hover:text-white rounded-xl text-xs font-bold">Tutup Soal Ini (Selesai)</button>
                            </form>
                        </div>
                    @else
                            ...
                            <div class="flex justify-between items-center p-3 bg-slate-950 border border-slate-800 rounded-xl">
                                ...
                                @if($rq->status === 'unused')
                                    <form action="/admin/rooms/{{ $room->id }}/select-question" method="POST">
                                        ...
                                        <button class="px-3 py-1.5 bg-pink-600 hover:bg-pink-500 text-white font-bold text-xs rounded-lg">Tampilkan Soal</button>
                                    </form>
                                @endif
                            </div>
                        @endforeach
